...
FALCON LADDER COPY (information ladder, step 2; docs/cluster_log.md §5). A copy of rrf_gsplat/train_rrf.py kept in
rrf_gsplat/cluster/ so the shared trainer is untouched; without the flags below it behaves exactly as train_rrf.py.
  --mirror-lobes NPZ   with --lobes 1: the lobe axis of the Gaussians in npz['idx'] is FIXED to npz['axis'] (the
                       receiver->Gaussian direction at which the mirror law puts the transmitter's reflection); every
                       axis is frozen, only those Gaussians' lobe weights train, kappa clamped to [--lobe-kappa-min, max]
  --sh-extra NPZ       SH bands 4..--sh-extra-degree for the Gaussians in npz['idx'] only (torch path: gsplat caps SH
                       at degree 4), zero-initialised, clamped to [-1, 1] after every step
...
def sh_basis_any(deg, dirs):
    """Real SH to degree `deg`, INRIA ordering (m = -l..l) and signs (Condon-Shortley phase); equals sh_basis up to
    degree 3 (checked 7.8e-16) and gsplat's degree 4 (1.7e-15) -- output/cluster/ladder/explore_adaptive/code."""
    x, y, z = dirs[:, 0], dirs[:, 1], dirs[:, 2]
    C, S = [torch.ones_like(x)], [torch.zeros_like(x)]
    for m in range(1, deg + 1):
        c, s = C[m - 1], S[m - 1]
        C.append(x * c - y * s); S.append(x * s + y * c)
    Q = {}
    for m in range(deg + 1):
        dfact = 1.0
        for k in range(1, 2 * m, 2):
            dfact *= k
        Q[(m, m)] = torch.full_like(z, (-1) ** m * dfact)
        if m + 1 <= deg:
    ...
    for l in range(deg + 1):
        for m in range(-l, l + 1):
            ...
            if m == 0:
                ...
                cols.append(math.sqrt(2) * K * Q[(l, am)] * C[am])
            else:
                cols.append(math.sqrt(2) * K * Q[(l, am)] * S[am])
...
class RRF(torch.nn.Module):
    ...
    def add_sh_extra(self, idx, degree):
        """--sh-extra (ladder copy): SH bands 4..degree for the Gaussians in idx only, zero-initialised."""
        assert self.sh_degree == 3 and degree >= 4, "--sh-extra adds bands above degree 3 to an SH3 model"
        dev = self.params["means"].device
        self.sh_extra_idx = torch.as_tensor(np.asarray(idx), dtype=torch.long, device=dev)
        self.sh_extra_degree = int(degree)
        self.params["shH"] = torch.nn.Parameter(torch.zeros(len(self.sh_extra_idx), (degree + 1) ** 2 - 16,
                                                            self.channels, device=dev))

    def set_mirror_lobes(self, idx, axis):
        """--mirror-lobes (ladder copy): after add_lobes(1, kappa), fix the lobe axis of the Gaussians in idx to the
        mirror-law direction and freeze every axis; lobe_mask marks the Gaussians whose lobe weight may train."""
        dev = self.params["means"].device
        idx = torch.as_tensor(np.asarray(idx), dtype=torch.long, device=dev)
        ax = F.normalize(torch.as_tensor(np.asarray(axis), dtype=torch.float32, device=dev), dim=-1)
        with torch.no_grad():
            self.params["lobe_axis"][idx, 0, :] = ax
        self.params["lobe_axis"].requires_grad_(False)
        self.lobe_mask = torch.zeros(self.n_gaussians, device=dev)
        self.lobe_mask[idx] = 1.0

    def _sh_colours(self, cam_center, means=None, sh=None, frozen=None):
        col = self._sh_colours_base(cam_center, means, sh, frozen)
        if means is None and sh is None and "shH" in self.params:      # the main Gaussians, not the emitters
            idx = self.sh_extra_idx
            with torch.no_grad():
                d = F.normalize(self.means.detach()[idx] - cam_center[None], dim=-1)
                b = sh_basis_any(self.sh_extra_degree, d)[:, 16:]       # bands 4..L
            col = col.index_add(0, idx, (self.params["shH"] * b[:, :, None]).sum(1))
        return col

    def _sh_colours_base(self, cam_center, means=None, sh=None, frozen=None):
    ...
    def load_state(self, st):
        ...
        for k in ("lobe_axis", "lobe_logk", "lobe_w", "pc_latent", "em_opacities", "em_sh0", "em_shN", "em_pc_latent", "shH"):
    ...
    def state(self):
        ...
        if "shH" in self.params:
            st["sh_extra_idx"] = self.sh_extra_idx; st["sh_extra_degree"] = self.sh_extra_degree
...
def main():
    ...
    ap.add_argument("--mirror-lobes", default=None, help="npz (idx, axis): mirror-law lobe axes, frozen (ladder copy; needs --lobes 1)")
    ap.add_argument("--lobe-kappa-min", type=float, default=30.0)
    ap.add_argument("--lobe-kappa-max", type=float, default=3000.0)
    ap.add_argument("--sh-extra", default=None, help="npz (idx): SH bands 4..--sh-extra-degree for these Gaussians (ladder copy)")
    ap.add_argument("--sh-extra-degree", type=int, default=6)
    ...
    if cfg.lobes:
        ...
        if cfg.mirror_lobes:
            if cfg.lobes != 1 or cfg.init_from:
                raise SystemExit("--mirror-lobes: --lobes 1 and no --init-from")
            ml = np.load(cfg.mirror_lobes)
            model.set_mirror_lobes(ml["idx"], ml["axis"])
            print(f"mirror lobes: {len(ml['idx'])} Gaussians with mirror-law axes (all axes frozen), "
                  f"kappa in [{cfg.lobe_kappa_min:g}, {cfg.lobe_kappa_max:g}]")
        if cfg.init_from:
        ...
        print(f"lobes: {cfg.lobes} per Gaussian, kappa {cfg.lobe_kappa:g} (width ~{math.degrees(cfg.lobe_kappa ** -0.5):.0f} deg), "
              f"weights from zero")
    if cfg.mirror_lobes and not cfg.lobes:
        raise SystemExit("--mirror-lobes needs --lobes 1")
    if cfg.sh_extra:
        model.add_sh_extra(np.load(cfg.sh_extra)["idx"], cfg.sh_extra_degree)
        print(f"SH bands 4..{cfg.sh_extra_degree} for {len(model.sh_extra_idx)} Gaussians (torch path), clamped to [-1, 1]")
    if cfg.pcolor:
        rx = torch.linalg.inv(train["viewmats"])[:, :3, 3]
    ...
    lrs = {"sh0": cfg.feature_lr, "shN": cfg.feature_lr / 20.0, "opacities": cfg.opacity_lr,
           ...
           "em_pc_latent": cfg.feature_lr, "shH": cfg.feature_lr / 20.0}
    ...
    for it in range(1, cfg.iterations + 1):
        ...
        if getattr(model, "lobe_mask", None) is not None and model.params["lobe_w"].grad is not None:
            model.params["lobe_w"].grad.mul_(model.lobe_mask[:, None, None])      # --mirror-lobes: selected only
        for opt in optimizers.values():
            opt.step()
        if getattr(model, "lobe_mask", None) is not None:
            with torch.no_grad():
                model.params["lobe_logk"].clamp_(math.log(cfg.lobe_kappa_min), math.log(cfg.lobe_kappa_max))
        if "shH" in model.params:
            with torch.no_grad():
                model.params["shH"].clamp_(-1.0, 1.0)
        if strategy is not None:
