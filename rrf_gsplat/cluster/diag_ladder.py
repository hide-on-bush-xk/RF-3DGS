...
import train_rrf_ladder as T  # noqa: E402  (ladder copy)
...
def main():
    ...
    ap.add_argument("--names-file", default=None, help="score these views instead; protocol training views only (ladder copy)")
    ap.add_argument("--out-tag", default="trainfit", help="outputs in <run>_<tag>/ and <run>_<tag>.json (ladder copy)")
    a = ap.parse_args()
    ...
    if a.names_file:
        ...
        assert set(names) <= set(train_names), "--names-file must list protocol training views only"
    dev = "cuda"
    ...
    if "shH" in st:                       # --sh-extra (ladder copy): index set and degree come from the state
        model.add_sh_extra(st["sh_extra_idx"].cpu().numpy(), int(st["sh_extra_degree"]))
    model.load_state(st)
    out = os.path.join(a.run + "_" + a.out_tag, "renders")
    with torch.no_grad():
    ...
    json.dump(rep, open(a.run + "_" + a.out_tag + ".json", "w"), indent=1)
