#!/bin/bash
# Round 46b: round 46 again after the lm_optim memory-leak fix (161ae09), for a valid time only: round 46 spilled
# past the card (sysmem fallback) in LM iterations 4-5. Same configuration, seeds and decision.
...
# r45_lm and "adapted trust region" = r46b_lmr):
...
export PYTHONUTF8=1
LOG=$REPO/output/rrf/win_round46b.log
cd $REPO
...
echo "== round 46b start $(date +%H:%M:%S)" >> $LOG
for s in 0 1 2; do
  sfx=""; [ $s -gt 0 ] && sfx="_s$s"
  run r46b_lmr$sfx --sh-degree 3 --seed $s --lm-after 1600 --lm-iters 5 --lm-radius 1e4 --lm-radius-max 1e16 --bwd-no-geom off
done
