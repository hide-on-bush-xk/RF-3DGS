#!/bin/bash
# Round 53b: round 53 in --mode power. Round 53 trained in db mode, which alpha-composites dB values: the power
# splat is additive in LINEAR power, not in dB, and its -200 dB floor (no path) puts 50+ dB edges into a dB-space L1
# fit, so round 53 did not test additivity (its result, 14.7 % <= 1 deg, is recorded as confounded). --mode power
# composites linear powers and reads back dB. Same data, benchmarks and reading as below.
...
export PYTHONUTF8=1
LOG=$REPO/output/rrf/win_round53b.log
cd $REPO
echo "== round 53b start $(date +%H:%M:%S)" >> $LOG
...
B="--mode power --source $POWG --sh-backend gsplat --eval-group --faces-per-step 4 --lr-scale 2 --save-renders -1 --sh-degree 3 --seed 0"
if [ ! -f output/rrf/r53b_pow_fit_160/results.json ]; then
  mkdir -p output/rrf/r53b_pow_fit_160
  $PYW rrf_gsplat/train_rrf.py --out output/rrf/r53b_pow_fit_160 $B --iterations 3000 --eval-every 3000 --max-train-views 640 > output/rrf/r53b_pow_fit_160/train.log 2>&1
fi
echo "r53b_pow_fit_160: $(tail -1 output/rrf/r53b_pow_fit_160/train.log | cut -c1-100)" >> $LOG
$PYW rrf_gsplat/diag_train_fit.py --run output/rrf/r53b_pow_fit_160 --train-subset 640 >> $LOG 2>&1
if [ ! -f output/rrf/r53b_pow_full/results.json ]; then
  mkdir -p output/rrf/r53b_pow_full
  $PYW rrf_gsplat/train_rrf.py --out output/rrf/r53b_pow_full $B --iterations 2500 --eval-every 2500 > output/rrf/r53b_pow_full/train.log 2>&1
fi
echo "r53b_pow_full: $(tail -1 output/rrf/r53b_pow_full/train.log | cut -c1-100)" >> $LOG
$PYW rrf_gsplat/diag_train_fit.py --run output/rrf/r53b_pow_full --positions 160 >> $LOG 2>&1
[ -d output/rrf/t4pow_nn ] || $PYS sionna_port/t4_baselines.py --truth $POWG --which nn --out-prefix output/rrf/t4pow_ >> $LOG 2>&1
$PYS sionna_port/t4_score.py --truth $POWG --runs output/rrf/r53b_pow_full output/rrf/t4pow_nn --cov output/rrf/t4_rt_cov_all.npz \
    --out output/rrf/t4_scores_pow_powermode.json >> $LOG 2>&1
