"""Smoke for --pcolor (P7, a position-conditioned colour residual; train_rrf.RRF.pcolor).

Criteria, written before the first run:
  a  at initialisation (the MLP's last layer zero) the renders equal the plain SH3 model's bit for bit, on 2 positions x 4 faces
     (every orientation) spread over the held-out list
  b  after one Adam step the latent and every MLP tensor have a nonzero, finite gradient (the latent and the
     inner layers only once the last layer is nonzero, hence after one step)
  c  300 steps on 2 training positions (8 views) with --pcolor 8: no NaN, final loss below the first
  d  one training step (4 faces, render + loss + backward + Adam) costs <= 2 x the SH3 step at 300 x 200
     (CUDA events, 20 steps after 5 warm-up, median; not a timing-contract measurement, no quiet gate)

    C:/Users/Ke/miniconda3/envs/rf-gsplat-win/python.exe rrf_gsplat/check_pcolor.py
"""
...
def main():
    ...
    res = {}
    # a
    m0, m1 = model(0, dev, rx), model(8, dev, rx)
    with torch.no_grad():
        worst = 0.0
    ...
    opt = {k: torch.optim.Adam([p], lr=lr, eps=1e-15) for k, p, lr in
           [(k, p, {"sh0": 5e-3, "shN": 2.5e-4, "opacities": 0.1, "pc_latent": 5e-3}[k])
            for k, p in m1.params.items() if p.requires_grad]}
    opt["mlp"] = torch.optim.Adam(m1.pcolor_mlp.parameters(), lr=2e-3)
    ...
    for it in range(300):
        ...
        loss.backward()
        if it == 1:
            tens = {"pc_latent": m1.params["pc_latent"]} | {f"mlp.{n}": q for n, q in m1.pcolor_mlp.named_parameters()}
            grads_ok = {k: (q.grad is not None and bool(torch.isfinite(q.grad).all()) and float(q.grad.abs().max()) > 0)
                        for k, q in tens.items()}
        for o in opt.values():
            o.step()
    ...
    # d: step time, SH3 vs SH3 + the position-conditioned colour (latent 8)
    def step_ms(m):
        ops = {k: torch.optim.Adam([p], lr=1e-3) for k, p in m.params.items() if p.requires_grad}
        if getattr(m, "pcolor_mlp", None) is not None:
            ops["mlp"] = torch.optim.Adam(m.pcolor_mlp.parameters(), lr=1e-3)
        ts = []
        for it in range(25):
    ...
    t0, t1 = step_ms(model(0, dev, rx)), step_ms(model(8, dev, rx))
    res["d_step_ms_sh3_pcolor8"] = [t0, t1]; res["d"] = t1 <= 2.0 * t0
