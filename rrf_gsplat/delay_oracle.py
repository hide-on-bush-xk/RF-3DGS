...
That caveat is the point of the word "partial": the number below is not a
bound on what better geometry could achieve, only on what swapping it in
after the fact is worth.
...
def stats(e, w):
    """Unweighted and power-weighted summaries of a signed error array.

    median_pw is the weighted median: sort by |error|, walk the cumulative
    weight and take the value at the halfway point. It answers "what does the
    error look like where the power actually is", which the plain median does not.
    """
...
def main():
    """Re-decode every held-out view with the mesh range term and compare."""
    ...
    # These two must match how the run was trained, or the swap subtracts a
    # different term from the one the residual was fitted against.
    ...
    # The two files must be view-for-view identical, or the swap would mix
    # one view's mesh depth into another's prediction.
    ...
    if cfg.range == "euclid":
        # sec(theta) per pixel: 1 on the principal ray, up to 1.41 at a 90-degree
        # face's edge. This is the z-to-range factor depth_gt.py also reports.
        u = (np.arange(w) + 0.5 - K[0, 2]) / K[0, 0]; v = (np.arange(h) + 0.5 - K[1, 2]) / K[1, 1]
        f = np.sqrt(1.0 + u[None, :] ** 2 + v[:, None] ** 2)
    else:
        f = np.ones((h, w))                   # camera z, as the original runs used
    ...
    def space_of(rx):
        """Which space a receiver sits in, with a 0.3 m tolerance on the walls.

        Falls back to the nearest space centre for a position that lands in no
        rectangle (a doorway, say), so every view gets a class.
        """
        for s in spaces:
    ...
    # Needs both a saved render and a depth entry; anything else is skipped.
    ...
    for n in test:
        ...
        swap = np.isfinite(z)                 # false where the mesh ray escaped
        orc = pred.copy()
        # Only the delay channel changes; every other channel is the model's.
        ...
        # gap_used records how much of the image the oracle actually touched:
        # a small share would make the comparison uninformative.
    ...
    for c in sorted(set(cls)):
        ...
        # Weights renormalised within the class, so each class's weighted median
        # is relative to its own power distribution.
    ...
    # model against oracle on the same pixels: the difference is what the
    # geometry costs at test time.
