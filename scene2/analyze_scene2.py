...
def load(name):
    """Read a JSON file under output/rrf, or None if that run has not been done.

    Returning None rather than raising is deliberate: every section below prints a
    "(missing)" line and carries on, so a partial set of runs still produces a
    usable report.
    """
...
def final(run):
    """The "final" block of a run's results.json, or None if the run is absent."""
    d = load(os.path.join(run, "results.json"))
    return d["final"] if d else None


def density():
    """Claim A: how copy and field compare as the training density falls.

    "copy" is the nearest training position's spectrum used verbatim; "field" is
    this method's prediction. The interesting quantity is where the two cross:
    dense enough and copying wins, sparse enough and the field does.
    """
    ...
    for scene, pat, runs in rows:
        ...
        for run, npos in runs:
            b = load(pat % run)
            if not b:
                # Named explicitly so a gap in the table can be traced to a run.
                print(f"  {npos:>9}  (missing {run})"); continue
            # "nearest" is the copy baseline, "rrf" is this method. `sp` is the
            # median distance to the nearest training position: the physical
            # spacing that the crossover is expressed in.
...
def density_by_space():
    ...
    Splitting by space class is the point: the corridor, the closed rooms and the
    open hall have different geometry, so a single pooled crossover would average
    over regimes that behave differently.
    """
    ...
    rows = [(n, b) for n, b in rows if b]       # drop runs that have not been done
    if not rows:
        print("\nA'. (no full-set baselines yet)"); return
    # Class list is the union over runs, so a class missing from one density still appears.
    ...
    for c in classes:
        ...
        pts.sort(key=lambda p: p["spacing"])    # ascending spacing, so the scan below is monotone
        def cross(q):
            """Spacing at which copy - field changes sign, by linear interpolation.

            Reported as a bound rather than a number when the sign never flips
            inside the measured range -- the honest answer when the crossover is
            outside what was actually sampled.
            """
            d = [(p["spacing"], p["copy"][q] - p["field"][q]) for p in pts]
            if d[0][1] > 0:
                return f"below {d[0][0]:.2f} m"     # field already ahead at the densest point
            for (x0, y0), (x1, y1) in zip(d[:-1], d[1:]):
                if y0 <= 0 < y1:
                    return f"{x0 + (x1 - x0) * (-y0) / (y1 - y0):.2f} m"
            return f"beyond {d[-1][0]:.2f} m"       # copy still ahead at the sparsest point
...
def zones():
    """Claim B: does transfer benefit depend on where the source transmitter sits?

    benefit = cold RMSE - transferred RMSE, so positive means the transfer helped.
    Sources are grouped by topology relative to the target room, and the benefit is
    additionally fitted against raw distance to see whether topology says anything
    that distance alone does not.
    """
    print("\nB. zone structure on room_S2 (in-range RMSE, 2k-step transfer; benefit = cold - transferred)")
    # cold = visual geometry only, no transfer; own = the target's own data, unfrozen.
    # These bracket what any transfer can achieve.
    ...
    # Hand-assigned topology, the independent variable of this section.
    ...
    for name, pos in tx.items():
        if name == "room_S2":
            continue                             # the target is not a source for itself
    ...
    # n is printed with each group mean: several groups have only one or two
    # sources, so the means are not all equally supported.
    ...
    if len(rows) >= 4:
        ...
        # Model: benefit = b0 * exp(-d / d_c) + c. Only d_c is non-linear, so it is
        # found by a grid scan while b0 and c come from least squares at each d_c.
        best = None
        for dc in np.linspace(0.25, 40, 800):
            ...
            if best is None or sse < best[0]:
                best = (sse, dc, coef)
        ss = float(((b - b.mean()) ** 2).sum()) or 1.0     # `or 1.0` guards a constant b
    ...
    # "facing" = the same room plus the room directly across the corridor, i.e. the
    # sources the zone hypothesis predicts should help; everything else is the control.
    ...
    out = {"scene": "scene 2 (corridor), target room_S2", "transfer_budget": "2k", "tx_b": tgt.tolist(),
           ...
           # min of the facing group against max of the other group: if those two do
           # not overlap, the groups are separated without needing a test statistic.
...
def overlap(rows):
    ...
    Returns {source_name: (lit_iou, db_corr)} for the caller to fold into its JSON.
    An absent target directory returns {} rather than failing the whole report.
    """
    ...
    # -300 dB is the sentinel written for a receiver with no paths at all; such a
    # view has no "brightest pixel" and would otherwise dominate a per-view threshold.
    t_empty = tgt.reshape(len(names), -1).max(1) <= -299
    # "strong" is per view (relative to that view's own peak); "lit" is per dataset
    # (relative to the global peak). The two answer different questions and are
    # both reported because they can disagree.
    ...
    for name, g, d, r, b in sorted(rows, key=lambda r: r[2]):
        ...
        keep = ~(t_empty | s_empty)             # views where both sides have signal
        s_strong = src > (src.reshape(len(names), -1).max(1)[:, None, None] - 30.0)
        s_lit = src > (src.max() - 60.0)
        # max(..., 1) guards an empty union, which would otherwise divide by zero.
        ...
        u = (s_lit | t_lit)
        # Correlation of the dB values themselves, on the pixels either side lights.
    ...
    if len(ious) >= 4:
        # Does any overlap measure predict the transfer benefit? Three candidates,
        # correlated against benefit over however many sources exist.
...
def consistency():
    """Claim C: two protocol variants, checked on both scenes.

    Each pair is (baseline run, variant run) scored on one key, printed as
    "before -> after" so a regression is visible without a separate delta column.
    """
    ...
    for scene, pairs in (("scene 1", [("per-view normalisation (MVDR rgb)", "e2_mvdr_rgb", "e1_mvdr_perview", "psnr_rgb"),
        ...
        for label, a, b, key in pairs:
            ...
            if fa and fb:
                ...
                # Both runs are needed for a comparison; one alone says nothing.
