...
**Scope.** One lobby, two carrier frequencies (2.4 and 60 GHz), two material sets, one array (a 10-element uniform planar array, as in the released data), one visual checkpoint. A second scene, a corridor with six closed rooms and a hall, replicates three claims with predictions written before the runs (§9): the density crossover and the consistency costs keep their form with scene-dependent sizes, and the zone structure of geometry transfer becomes an order of magnitude sharper.
...
**Prediction 1 held in substance and failed in its grouping.** The exponential fit fails outright (R² 0.14; benefit against distance +0.11), the separation is far sharper than the lobby's (+2.95 to −12.73 dB against +0.57 to −0.35), and the lit-surface overlap between rooms is zero. But the partition written before the run (same room / adjacent / opposite / corridor / hall) is not the operative one: the room next door, sharing a wall, is as harmful as the diagonal room (−8 to −12 dB), while the room facing the target door to door across the corridor helps (+2.95, more than the same-room source's +2.55; the seed noise floor was not measured on this scene, and the 0.4 dB is not interpreted). What separates the signs is whether the source lights any of the target's surfaces (IoU above 0.01: five sources from +2.95 to −0.19; IoU zero: seven sources from −1.56 to −12.73), not adjacency and not distance (the nearest room source, 3.7 m away, is −11.4; the facing room at 7.3 m is +3.0). This is the lobby's finding (benefit tracks the overlap of lit surfaces) pushed to its limit by closed rooms: two rooms facing each other share the corridor walls and each other's door openings; two rooms sharing a wall share nothing. The negative plateau is an order of magnitude deeper than the lobby's −0.46 dB: a geometry adapted inside one closed room is worse than the visual geometry by 8–13 dB in another. Part of that size is the metric: the in-range RMSE counts the 224 held-out images in which the target has no path, so an adapted geometry that leaves anything in those views is penalised on half the test set; the mechanism was not measured. For deployment the lobby's rule becomes a necessity: reuse an adapted geometry by lit-surface overlap only; reuse by distance or by room adjacency costs 8–13 dB.

`[MISSING: the extrapolation claim (prediction 3, a whole held-out room) was not run on the corridor.]`

## 10. Limits

One lobby with two carriers, two material sets and one array, and one procedural corridor on which three claims were replicated (extrapolation into a whole held-out room was not run; the corridor's seed noise floor was not measured). All data are simulated; the consistency result is about the simulator's sampling, and a measured dataset has its own inconsistency (calibration drift across a survey) that we did not model. MCMC densification is untuned and listed as a negative result; end-to-end differentiability to materials was not attempted; a jointly trained shared deformation basis is untested; the delay residual's remaining 4.7 ns single-path RMSE has no mechanism yet; the M = 64 spectra of the original paper's headline array were not regenerated (the ported beamformer caches an [M², H, W] grid and exceeds 12 GB). The transmitter-transfer timings were taken on a shared GPU and are not reported.
