...
- **Path-statistics five-channel target** (MULTI): each ray-traced path is splatted with a Gaussian kernel (σ = 3 px = 1.0° unless stated) at its arrival direction into five channels: path power in dB, cos and sin of the departure azimuth, departure zenith, delay. Where several paths land in one pixel the channels hold the power sum and the power-weighted circular mean azimuth, mean zenith and mean delay: the first moments of the paths in that pixel, not the multipath channel. Two equal-power paths in one pixel give one label; the target does not represent a multi-peak departure spectrum, a power-delay profile, coherent superposition or phase, and nothing in this paper is a channel-impulse-response or CSI reconstruction. The path power is the squared sum over the array's elements of the path coefficient's magnitude, (Σ_m |a_m|)². Only when the element magnitudes are equal, as they are here for a synthetic array whose elements share one pattern and see one far-field path, is this the single-element path power times a constant (M⁴); in general it is neither an array-averaged nor a combined power, carries no inter-element phase, and is the output of no receiver. It is a stable supervision target and is used as nothing more. This is the target on which angles and delays are decoded (§7).
...
Decoded azimuth, zenith and delay are computed on the pixels a path reaches (the "hit" support, power above the −150 dB kernel floor), per pixel, and reported as median / P90 / RMSE. The tail (a few percent of pixels where two paths overlap) dominates the RMSE and the median is the headline; both are always given. Power-weighted variants weight each pixel by its linear power. The dB RMSE inside the dataset's range and the PSNR after jet mapping are reported as compatibility numbers. Three splits: held-out positions (20 %, random over the route), a contiguous held-out region (§7.4), and a training-density sweep with the lookup baseline drawn from the same subset (§7.3). Every baseline is scored on exactly the pixel set of the model it is compared with; runs whose saved renders covered a subset of the held-out views were voided and re-rendered on the full set before scoring (§10.4). A constant predictor (the training pixels' mean) is reported for every channel in every regime: where it beats the field, the channel carries little information in that regime, and the field's loss is read as such rather than as a failure.

### 3.5 Reporting

Timings give the full chain (generation + training + evaluation), the GPU (RTX 3060, 12 GB, shared between Windows and WSL) and whether it was exclusive; a period during which another process shared the card voided every timing taken then, and those were re-measured. Every baseline appears as "as published" and "as published + engineering fixes"; speed is not a contribution of this paper and the cost table is reported under the contract, not argued from. Seed noise floors: 0.03 dB on the transfer benefit (four seeds, lobby), 0.31 dB per transfer on the corridor, 0.002–0.026° and 0.006–0.011 ns on the decoded medians of the lobby density sweep (four seeds at two levels). The criterion of every experiment in §4–§7 was written down before the run; a prediction that failed is reported as failed (§7.5, §7.4).
...
**Data versions.** The generator changed twice during the work (per-view seeding → per-position seeding → kernel truncation at −150 dB). Every MVDR / CBF result was checked to be insensitive to the seeding change (§4, third row), so those series were not re-baselined; every path-statistics result in §7 is on the per-position-seeded, truncated generator.
...
| per-view Monte-Carlo sampling of diffuse paths (the tutorial seeds once and draws the scattering subset per call, so the four faces of one position see four path sets) | geometric consistency | path-statistics target, in-range power RMSE 12.67 → 8.57 dB (−32 %) |
| the same seeding change on the beamformed MVDR target | — | 18.75 / 3.91 → 18.69 / 3.94 dB: nothing (noise floor 0.03 dB) |

The per-image normalisation numbers are the best case for the published practice: the evaluation was given the oracle range, which at deployment is not in the image at all. The third row bounds the claim: an MVDR spectrum is an array statistic over all sampled paths (a covariance inverse), and different diffuse subsets give nearly the same statistic; a path-statistics splat is the path set itself. Cross-view consistency therefore costs according to the target type, not the representation.

For scale, the representation changes measured on the same MVDR data are: dB target instead of jet RGB, +0.6 dB PSNR and −11 % RMSE (18.15 → 18.75 / 4.42 → 3.91; reaching 17 dB in 19 s instead of 69 s); SH degree 0 → 3, +2.0 dB; training the opacities, +0.2 dB; unfreezing the geometry, +2.8 dB (§5). The seeding effect on the path-statistics target (−32 %) is larger than the geometry effect (−27 %) on the same kind of number. The 12.9 dB power RMSE of our first path-statistics dataset and the 8.6 dB of the current one differ almost entirely by the seeding rule (12.67 → 8.57 with the encoding, channel count and range held fixed); encoding accounts for 0.02 dB.
...
Means move 6 mm (median); the gain is in scales and rotations. The adaptation is therefore a per-transmitter reshaping of the projection kernels and of visibility on a fixed set of positions, which compensates the target's error where the visual geometry composites the wrong surfaces; it is not a recovery of RF surfaces and is not called one here. In linear-power compositing the gain is the same (−27.5 % against −27.1 % in dB), so it is not the geometry compensating a dB-domain mixing bias; each ray averages 6–8 Gaussians (N_eff median 5.9 frozen, 7.5 unfrozen, Rademacher estimate), so the two compositing rules are not trivially identical either.
...
The four sources on B's side of the lobby (x ≥ 5 m) all help (+0.12 to +0.57, mean +0.33 dB); the six elsewhere all hurt (−0.06 to −0.35, mean −0.20 dB), whatever the distance: a source 5.3 m away in the hall is as useless as one 12.8 m away. Benefit correlates −0.81 with distance, +0.85 with the overlap of strongly lit surfaces and +0.77 with the correlation of the two transmitters' dB spectra; on this route distance and overlap are collinear, so this scene alone cannot name the variable. An exponential-plus-plateau fit in distance (correlation length 5.3 m) is recorded for completeness and withdrawn as a model: its bootstrap 90 % interval is 2 to 40 m, 17 % of resamples find no decay, and §10.2 shows on a scene that separates the two variables that distance is only a proxy for overlap. The plateau, −0.46 dB (bootstrap median −0.50), is the firm number: an adapted geometry carried where it does not belong is worse than the visual one.
...
**Table 6. Decoded error of the path-statistics target (640 held-out images, hit pixels), encodings compared.**
...
| --- | --- | --- | --- | --- | --- | --- |
| all 640 (the 800-position route minus the held-out 160) | 0.23 / 0.36 / 0.60 m | 1.07° / **0.62°** | **0.79°** / 0.93° (four-seed mean; seed 0 gave 0.96°) | **0.86** / 0.95 | 6.3° / 6.6 ns | 16.14 |
...
Lookup degrades linearly with spacing (azimuth 1.07° → 4.68°, ×4.4 from 0.23 to 0.84 m); the field is nearly flat (0.62° → 0.84°, ×1.3; delay 0.95 → 1.43 ns). By linear interpolation over four-seed means the crossovers are below 0.23 m for azimuth (the field already wins at the densest sampling by 0.45°, 225 seed standard deviations), 0.26 m for zenith and 0.25 m for delay: beyond about 0.3 m spacing the field beats lookup on all three physical channels. The seed noise floor of the decoded medians was measured at these two levels (four seeds each): 0.002–0.016° on azimuth, 0.018–0.026° on zenith, 0.006–0.011 ns on delay, against level-to-level steps of 0.4–0.5° and 0.4–0.5 ns, so the crossovers carry two digits (zenith 0.26 ± 0.005 m, delay 0.25 ± 0.002 m); lookup has no seed. They are estimates for this scene, this route and this target, not thresholds. The field's tail does not depend on density (P90 azimuth 6–7° at every level; copy 131–152°). Twenty positions, eighty images, give a 0.84° median azimuth.
...
| expected depth Σ wᵢ dᵢ / α | −0.64 ns | 0.88 / 5.38 / 5.26 | 0.39 / 2.02 | 10.7 % (67 %) | 0.63° / 6.86° | 1.00° / 14.1° |
| expected depth × sec θ_pixel (Euclidean range; default) | −0.48 ns | **0.55 / 4.48 / 4.97** | **0.23 / 1.45** | 9.1 % (65 %) | 0.64° / 6.94° | 1.01° / 14.3° |
| nearest copy (0.23 m), for reference | — | 0.86 / 44 / 20 | 0.44 / 1.01 | — | 1.07° / 131° | 0.79° / 90° |

Median 2.42 → 0.88 ns (2.8×) with the expected depth and 0.55 ns (4.4×) with the Euclidean range, P90 12.8 → 4.5, RMSE 8.9 → 5.0; the power-weighted median 3.54 → 0.23 ns, below lookup's 0.44. Power, azimuth and zenith are unchanged within 0.04° at the median (PSNR 16.13 → 16.14); the angle tails pay 0.6° / 1.3° at P90 for the expected-depth variant because the shared opacities now also serve the range term. In extrapolation (Table 9) the decomposition removes 3.71 ns of median error against 1.47 ns in interpolation, but the ratio (2.2×) is smaller than in interpolation (2.5×) because the learned residual degrades out of the training region while the analytic term does not. We had predicted the ratio would be larger; the mechanism we had written down concerned the absolute quantity, and the criterion was written in the wrong unit; both are reported.

Two predictions about the remaining tail were tested and one failed. The tail is not multi-path mixing: stratified by paths per pixel, single-path pixels (55 % of pixels) carry 55 % of the squared error after decomposition (median 0.86, RMSE 4.67 ns) while pixels with six or more paths have the lowest RMSE (3.54 ns); the convex-combination bias that explains the azimuth tail does not explain the delay tail. The tail is negative (65–69 % of the pixels with more than 5 ns error predict too short), consistent with the accumulated depth Σ wᵢ dᵢ being shortened by (1 − α) where the pixel's opacity is below one (10 m at α = 0.95 is −1.7 ns); normalising to the expected depth halves the bias (−1.10 → −0.64 ns) and removes a quarter of the RMSE. Two measurements against the scene mesh (Table 17) name the remaining sources. The Gaussians' expected depth is shorter than the mesh by 0.219 m on average in the lobby, which is −0.73 ns and the size and sign of the channel's residual bias (−0.64 ns): the negative tail is the geometry sitting in front of the surface, not the residual's fit. And the range term as implemented adds the camera z of the composited depth, whereas a path's length is the Euclidean range z / cos θ along the pixel's ray; on a 90° face the factor reaches 1.41 at the edge and 1.56 at the corner, and the gap (range − z) / c on the lobby's held-out pixels is 2.18 ns mean, 4.59 ns P90 and 18.8 ns at most, a known per-pixel factor that the learned residual had to absorb. Together with the geometry's own 2.0 ns depth RMSE this accounts for the single-path residual. Multiplying the depth by the per-pixel secant is one line; its criterion, written before the run, was a delay RMSE below 4.5 ns with the angles unchanged. It passed at the median (0.88 → 0.55 ns) and at P90 (5.38 → 4.48) and failed at the RMSE (5.26 → 4.97 ns), with the angles unchanged within 0.01°; the criterion is not moved. The secant is a per-pixel systematic term and repairs the median-scale error (the power-weighted median falls from 0.39 to 0.23 ns, below lookup's 0.44 for the first time); the RMSE is carried by a tail that stays 65 % negative and belongs to the geometry in front of the surface (2.0 ns depth RMSE) and to two-path pixels. The signed mean moves from −0.64 to −0.48 ns rather than to the geometry's own −0.73, so the learned residual had absorbed more than the secant gap; the residual bias is geometric in sign and about −0.5 ns in size. How much the residual absorbs is measured directly: replacing the rendered depth by the mesh depth at evaluation shifts the signed error by +0.92 ns in the lobby and +1.76 ns in the corridor, the size of each scene's geometric bias (−0.73 and −1.60 ns), and worsens the error, because the residual was trained to cancel that bias and now cancels a bias that is gone. The residual is a geometry compensator, in mean and per pixel; this is the second, independent confirmation of Table 17. Training the residual against the mesh depth would measure what a true mesh buys, with supervision unavailable at deployment, and is deliberately not pursued; the swap result is the statement kept. What the decomposition claims is therefore bounded: the analytic term corrects a known systematic error (the range, and its per-pixel geometry), and the remaining tail is carried by mechanisms this paper measures but does not decompose further, the geometry in front of the surface and the two-path pixels. The Euclidean-range, expected-depth variant is the method's default; the density and region tables use the accumulated-depth, camera-z variant, whose median differs from the expected-depth one by 0.07 ns.

RF-PGS computes time of flight purely from the geometry, the stronger and fully analytic form of the same idea, and reports no delay error; what is done here is the weaker, practical version, the analytic range kept and only the scattering residual learned, on a target whose delay is a per-pixel mean. BiWGS and RxGS do not encode delay; WiNeRT reports below 0.33 ns with a neural ray tracer on a different scene. The contribution is the measured decomposition and its floor and ceiling, not a first physical delay model.
...
## 8. The released benchmark on its own coordinates

Everything above varies one factor at a time on regenerated data. This section asks the question the field asks: on RF-3DGS's released datasets, with its released 2560 / 640 split and its own metrics code (the fork's `metrics.py`: PSNR, SSIM and VGG-LPIPS, per image, averaged over the 640 held-out views), does the trainer of §3 beat the released models? Every row is scored by the identical implementation from rendered PNGs, including ours, and the retrained-fork row is the control that the comparison is between methods and not between runs. The released data ship only jet pictures, so the value target of these rows is the jet-inverted PNG in [0, 1]; it has no meaning for the three-channel AoD and Delay encodings, marked N/A. The NeRF-family baseline (NeRF²'s own network and spectrum renderer, fed the pinhole rays of each receiver pose, 30k iterations of its published schedule) is row B.

**Table 20. RF-3DGS released benchmark: PSNR / SSIM / LPIPS on the 640 held-out views by the fork's metrics.py. A1 retrains the release with the fork's own train.py from the visual checkpoint (30k → 40k, the released recipe). Best cell per column in bold.**

| row | MVDR | CBF | TCBF | AoD | Delay | MPC |
| --- | --- | --- | --- | --- | --- | --- |
| A0 released checkpoint (40k) | 16.02 / 0.731 / 0.391 | 14.13 / 0.723 / 0.446 | none released | 19.34 / 0.681 / 0.394 | 17.73 / 0.691 / 0.380 | 15.51 / 0.628 / 0.385 |
| A1 fork's train.py, retrained here | 16.02 / 0.731 / 0.391 | 14.13 / 0.723 / 0.446 | 12.04 / 0.698 / 0.427 | 19.34 / 0.681 / 0.394 | 17.73 / 0.691 / 0.380 | 15.51 / 0.628 / 0.385 |
| A2 ours, jet target, frozen geometry, 10k | 15.97 / 0.727 / 0.397 | 13.97 / 0.712 / 0.459 | 12.04 / 0.686 / 0.444 | 19.26 / 0.677 / 0.393 | 17.68 / 0.689 / 0.380 | 15.45 / 0.625 / 0.388 |
| A3 ours, value target, frozen, 10k | 16.46 / 0.752 / 0.363 | 14.21 / 0.718 / 0.428 | 11.97 / 0.695 / 0.432 | N/A | N/A | 15.07 / 0.636 / 0.383 |
| A4 ours, jet target, unfrozen geometry, 10k | 18.35 / 0.777 / 0.314 | 13.40 / 0.753 / 0.424 | 11.36 / 0.734 / 0.361 | **20.13 / 0.713 / 0.359** | **18.68 / 0.726 / 0.334** | **16.59 / 0.672 / 0.347** |
| A5 ours, value target, unfrozen geometry, 10k | 19.43 / 0.817 / 0.277 | **15.07 / 0.749 / 0.388** | **13.35 / 0.742 / 0.339** | N/A | N/A | 16.08 / 0.670 / 0.347 |
| A2′ ours, jet target, frozen, 40k | 16.78 / 0.751 / 0.372 | — | — | — | — | — |
| A5′ ours, value target, unfrozen, 40k | **20.36 / 0.834 / 0.253** | — | — | — | — | — |
| B NeRF² (their model, pinhole rays), 30k, 3.4–3.8 h each | 16.75 / 0.698 / 0.430 | 13.68 / 0.654 / 0.516 | 12.18 / 0.611 / 0.558 | N/A | N/A | 12.57 / 0.506 / 0.517 |

Three readings. First, the control holds: retraining the release with its own code reproduces every released checkpoint to the third decimal, and our trainer on the same jet target is within 0.05–0.16 dB of it at 10k steps (A2) and 0.76 dB above it at the release's 40k (A2′), so the rasteriser and the training loop add nothing by themselves. Second, every spectrum has a row that beats the release on all three metrics at once: MVDR by 3.4 dB PSNR at 10k and 4.3 dB at 40k (0.10 SSIM, −0.14 LPIPS), CBF by 0.9 dB, AoD by 0.8, Delay by 1.0, MPC by 1.1, and TCBF, for which no model was released, by 1.3 dB against the retrained fork. Third, the decomposition the contract asks for: unfreezing the geometry is the common source of the gain on all six spectra (+2.3 dB on MVDR by itself); the value target adds 0.4 dB on MVDR alone and, on the per-image-normalised CBF and TCBF, turns geometry adaptation from a PSNR loss (−0.7 dB with the jet target, while SSIM and LPIPS improve) into a gain (+0.9 and +1.3 dB), which is §4's consistency mechanism on the authors' own coordinates; on the sparse MPC picture the jet inverse costs 0.4–0.5 dB and the jet target with unfrozen geometry is the best row. Fourth, the NeRF-family row: NeRF² with its published network, renderer and schedule reaches the release's PSNR on MVDR (+0.7 dB) and TCBF (+0.1 dB against the retrained fork) but is below it on CBF (−0.5 dB) and MPC (−2.9 dB), is worse than every INRIA row on SSIM and LPIPS, and needs 3.4–3.8 h per spectrum on the exclusive card against 6 min for A5, so on these coordinates our best row leads NeRF² by 2.7 (MVDR), 1.4 (CBF), 1.2 (TCBF) and 4.0 dB (MPC) on all three metrics at once. RF-PGS's 20.61 dB is on its own data and configuration average and is not placed in this table; on the released MVDR benchmark the best published number we can verify is the release's 16.02.

Timings for this table on the exclusive RTX 3060 (a quiet card; the round in which the two trainers overlapped other jobs was re-timed): 10k steps of our trainer on the released MVDR data take 230 s (43 it/s) and training plus the 640-view evaluation 276 s; the fork's 10k steps from 30k to 40k take 273 s, so at equal step count the two trainers cost the same on this size of picture and the 3.4–4.3 dB is not bought with time. NeRF² trains at 2.3–2.5 rays-batches per second and needs 12,053–12,769 s per spectrum plus 1.6 s per rendered view, 14.7 h for the four spectra.

The physical metric on this benchmark is Track D of the plan: the released AoD model reproduces its own target's departure angles with P90 errors of 37° (zenith) and 74° (azimuth), against 12° / 5.9° for our multi-channel field on our data (Table 6); the same field, trained on a multi-channel dataset generated at the released poses themselves (3200 views, 2.4 GHz, tutorial materials, 117 s of generation and 384 s of training), reaches P90 errors of 15.7° in zenith, 7.7° in azimuth and 4.6 ns in delay on the released held-out split, against 37° / 74° for the released AoD model. The released poses are denser than ours (0.23 m to the nearest training view against 0.44–0.60 m), so the interpolating baselines have small median errors there (0.8–1.1° in azimuth) and separate from the field only in the tail: nearest-view P90 is 133.6° in azimuth and 91.1° in zenith, two-view interpolation 11.9° and 65.8°, against 7.7° and 15.7° for the field, and 4.6 ns against 33.9–45.4 ns in delay. The constant baseline's 29.4° median azimuth error reproduces the known failure of the tutorial's RGB encoding (about 27°) and is the control that the decoder is wired correctly.

## 9. Cost under the contract
...
## 10. A second scene: a corridor with closed rooms as an instrument
...
### 10.2 Zone structure: distance and overlap dissociated (prediction 1)
...
### 10.3 Extrapolation into a whole room (prediction 3)
...
### 10.4 Density by propagation regime (prediction 2)
...
Three regimes, three answers. Along the corridor, a waveguide, lookup barely degrades (azimuth 1.31° → 1.63° from 0.38 to 0.68 m), the azimuth crossover is about 0.5–0.6 m (it rests on level-to-level gaps of 0.05–0.06°, three lobby-sized seed standard deviations; the corridor's own seed floor on decoded metrics was not measured), and on delay lookup wins at every spacing measured. In the hall, an open space like the lobby, the field wins all three channels at the densest level (0.90° / 0.52° at 0.48 m, the lobby's form: 2.55° / 0.66° at 0.51 m). In the rooms lookup degrades fastest (azimuth 2.69° → 20° from 0.44 to 0.99 m) and the angle crossovers lie below the densest level. Prediction 2 therefore resolves as: reversed in the waveguide, and unresolved in the rooms and the hall, whose crossovers are below 0.44–0.48 m and cannot be placed against the lobby's 0.23–0.35 m because the corridor scene has no denser level (its 0.2 m route step is erased by the generator's ±0.5 m jitter). The statement that survives both scenes is that the crossover is not a property of the scene but varies with the local propagation regime, ranging inside one building from below 0.44 m (rooms, hall) to about 0.5–0.6 m (corridor azimuth) to no crossover within the sweep (corridor delay); each number is an estimate for its scene, route and target. The field's delay along the corridor is where the geometry may show: grazing walls give the largest depth errors (Table 17), and the range term carries them into the delay. Whether the waveguide's lookup advantage on delay is the geometry's or the regime's could not be settled by a test-time oracle (swapping the rendered depth for the mesh depth at evaluation): the learned residual has absorbed the geometry's mean bias, so the swap double-counts it and worsens every run by about that bias (§7.5); a train-time oracle was not run, and the sentence is left as two open explanations. The constant predictor is far behind both in every regime at full density (azimuth 37° / 4° / 101° and delay 16 / 41 / 12 ns in the corridor / hall / rooms), so no channel is degenerate here; it wins only in the held-out room of Table 18, on zenith.

### 10.5 Consistency (prediction 4)
...
| per-view sampling of diffuse paths (path-statistics target), in-range power RMSE | 8.57 → 12.67 dB (+48 %) | 8.04 → 9.44 dB (+17 %) |
...
The corridor's seed noise floor was measured only on the zone experiment (0.31 dB per transfer; the lobby's 0.03 dB does not carry over, the effects being 30× larger and the noise 10×), not on the density sweep, whose level-to-level steps (1.85° → 2.72° in copy azimuth) are far above it; the dataset floor for receivers the transmitter cannot reach (written at −300 dB, 46–48 % of the room transmitters' views) enters the room datasets' range and the in-range RMSE of Table 16 but not the path-statistics tables, whose ranges and errors use lit pixels only; the density and region runs of the corridor use the accumulated-depth, camera-z range term, so their crossovers are internally consistent but not the best delay numbers.

## 11. Limits

One lobby with two carriers, two material sets and one array, and one procedural corridor on which four registered predictions were tested. All data are simulated; the consistency result is about the simulator's sampling, and a measured dataset has its own inconsistency (calibration drift across a survey) that we did not model. MCMC densification is untuned and listed as a negative result; end-to-end differentiability to materials was not attempted; a jointly trained shared deformation basis is untested; the delay residual's two named sources (the geometry in front of the surface, the camera-z range term) are measured and the second is corrected, but the corrected run's RMSE (4.97 ns) missed its pre-registered bar (4.5 ns), and a mesh-depth-supervised residual is deliberately not run; the M = 64 spectra of the original paper's headline array were not regenerated (the ported beamformer caches an [M², H, W] grid and exceeds 12 GB). The transmitter-transfer timings were taken on a shared GPU and are not reported.

## 12. Conclusion

In the RF-3DGS setting, how consistently the data pipeline observes the field across views matters more than any change to the representation for path-statistics targets and not at all for beamformed ones; geometry adaptation is a small, distributed, transmitter-specific correction that must not be shared across zones; the metrics in use see none of this, and decoded physical accuracy reported against a lookup floor does. The one algorithmic change that survived this protocol, the analytic delay range, came from the floor, not from a paper.
