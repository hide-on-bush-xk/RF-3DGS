...
**Table 14. Corridor density sweep (MULTI, 2.4 GHz, transmitter at the corridor's middle, delay decomposition; copy from the same training subset). Scored on the 300 of the 452 held-out images whose renders were saved; the full-set, per-regime numbers are Table 19. Medians; P90 in parentheses for the first and last rows.**
...
The spacing axis is the measured nearest-training distance in both scenes (the corridor route has a 0.2 m step, but the generator applies the tutorial's ±0.5 m horizontal and −1.0 / +0.3 m vertical jitter, so the densest corridor level is 0.47 m against the lobby's 0.23 m). Aggregated over the scene the crossovers sit at about 0.47 m (azimuth, a tie at the densest level), 0.50 m (delay) and below 0.47 m (zenith), a larger spacing than the lobby's 0.23–0.35 m, and both curves moved: lookup is better in the corridor at equal spacing (1.85° at 0.47 m against the lobby's 2.55° at 0.51 m) and the field is worse (1.84° against 0.62°). The field's factor is the geometry's (Table 17: 3.1× the depth error); the lookup's is the regime: the mechanism written before the run, that the crossover is set by how fast the multipath structure changes with position, predicts opposite signs for a waveguide and for a room, and the aggregate averages them. Split by the space class of the held-out receiver `[PENDING: Table 19, all 452 views scored per space class; on the 300-view subset the corridor receivers have copy 1.38° against field 3.55° at 0.62 m and the room receivers copy 2.69° against field 2.14° at 0.44 m]`. The form of the statement holds in both scenes: beyond a spacing set by the local propagation regime, the field beats lookup on all three physical channels, and its tail is its advantage (P90 azimuth 13–30° against 159–172°; the corridor's azimuth is bimodal, so copying a neighbour whose dominant direction is the opposite one costs 180°). The 28-position copy row is better than the 56-position one; that is the sampling noise of a 28-position subset and is reported as measured.
...
The seed noise floor of the corridor's zone experiment `[PENDING: three cold seeds and two seeds of the two positive transfers, running]`; the dataset floor for receivers the transmitter cannot reach (written at −300 dB, 46–48 % of the room transmitters' views) enters the room datasets' range and the in-range RMSE of Table 16 but not the path-level tables, whose ranges and errors use lit pixels only; the density and region runs of the corridor use the accumulated-depth, camera-z range term, so their crossovers are internally consistent but not the best delay numbers.
