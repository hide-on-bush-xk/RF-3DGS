...
- 方位角的极小点 σ* ≈ 2°(6 px):**RF 辐射场的表示带宽 ≈ 2° σ,折算 θ3dB = 2.355σ* ≈ 4.7°,等效阵列 M ≈ 102°/4.7° ≈ 22**。
  天顶中位数从 2° 起持平,P90 到 4.3° 仍在降;时延单调随平滑改善。
- 论文的 5.94° 波束指向中值误差对应的是 M=10 阵列(θ3dB ≈ 10°);这个表示在 2° 的核下解码中位 0.6°,尾巴 P90 5°。
- 注意事项:σ 越大,有路径的像素越多(核把功率抹到更多像素),各行的评估像素集不同;要做严格的"同支撑集"对比,
  应固定在 σ=1 的支撑集上评估。这一条列为待办。
