...
## 需要在干净数据(G3,按位置 seed)上重做的清单(按优先级)——round 14 之后的状态

1. **round 14 已跑**:e2_mvdr_db_posseed = 18.69 / 3.94 vs e2_mvdr_db 18.75 / 3.91——**波束谱族对 seed 方式不敏感**(差在噪声底上)。
   因此 G1 上的 MVDR/CBF 系列(E2、E1、E3、E4、解冻、子集、24/76、转移曲线)**不需要重立基线**;它们的数据版本只需标注。
2. **round 15 的条件重做按门(≥ 0.1 dB)不触发**:e2_mvdr_rgb_posseed、txB posseed、24/76 三条都不跑;db/rgb 差距被污染放大的担心在波束谱族上没有依据(污染本身量不到)。
   投影族(MULTI)的所有比较已经在 G2 内(cs 系列),不受影响。
3. 不重做、只标注:转移曲线(源版本混合,二元结论稳)、多通道 12.9 → 8.57(已归因)。
4. 解冻增益在干净数据上的重测(a_mvdr_db_geom_posseed)作为核对,不改变结论的前提。

## 对审稿人的一句话(修订)

所有 MVDR/CBF 头条实验的数据来自 G1(按视角 seed),与发布管线的构造一致;在同一设置上用按位置 seed 重生成并重训,结果在噪声底内不变(18.69/3.94 vs 18.75/3.91),
所以这一版本差异对波束谱族的结论没有影响;对路径级目标(MULTI)它值 4.1 dB,那些实验全部在修复后的版本上做。
