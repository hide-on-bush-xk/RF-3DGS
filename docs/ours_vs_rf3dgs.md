...
**状态(2026-09-20)**:RRF 训练在 gsplat 上完成并复现了锚点(第 7、8 节);规划端的材料反演仍是合成真值(inverse crime,不作为论文主张);实验在 NIST 大堂(2.4 / 60 GHz,两套材料)与一个走廊 + 封闭房间的第二场景上,depth 1。第 8 节是 09-19 之后的全部结果索引。
...
| 几何 | 冻结(视觉几何 = 射频几何) | 可解冻;24% 的收益跨 Tx 可沿用 | 解冻 RMSE −27%;其中可迁移 −5%(0.24 dB),76% Tx 专属;位置只动毫米级 |
...
- **几何解冻**:db 模型 18.75 / 3.91 dB → 21.53 / 2.85 dB,本阶段最大单项收益。位移诊断:位置中位数只动 6 mm、最大 21 cm,
  ...
  说明这不是"冻结 vs 联合"的差别。形状诊断:最短轴与表面法线的对齐变差(27° → 32°,否定"向 2DGS 靠拢"),
  旋转轴对 Tx 方向各向同性,沿 Tx 方向只多拉长 4%——Tx 专属但非方向性。
  **判别实验**:power 模式的解冻增益与 db 相同(−27.5% vs −27.1%),排除"几何在代偿 dB 域混合偏差";Tx-C 适配的几何搬到 Tx-B
  比视觉几何还差(4.95 vs 4.82),排除"通用容量再分配"。适配后的几何带 Tx 信息,只在 Tx 相近时可迁移。
  **对 Tx 泛化论据的含义**:高斯位置是 Tx 不变的基底;形状适配是 Tx 专属的;"颜色函数里没有入射方向"这个缺失维度的实测代价
  就是这 76%——Tx 条件化传输的立项依据成立,但形状诊断显示它不是朝 Tx 的简单拉伸,degree-1 的 Ω_in 是否够要靠留出 Tx 实验判。
  **谁在承载增益**:只解冻随机 1% 的高斯拿到 73% 的解冻增益,碟 11% 拿 99%、针 40% 拿 93%、随机 11% 拿 94%——形状类别不是变量,
  适配是低维、分布式的修正;每条射线上平均 6–8 个高斯(N_eff),dB 与线性功率合成增益相同是实测而非构造。
  **搬多远**(10 个源 Tx 的转移曲线,2k 步预算,噪声底 0.03 dB):**二元结构**——与 B 同侧的 4 个源全为正(+0.12…+0.57,均 +0.33),
  其余 6 个全为负(−0.06…−0.35,均 −0.20),与距离无关;指数拟合 d_c = 5.3 m 但自助法 90% 区间 2–40 m,只作参考。
  最硬的数是平台 **−0.46 dB**:别处适配的几何比视觉几何还差。10 组几何增量做 PCA:近乎两两正交(5 个主成分 66%),
  只有平均增量(26% 能量,≈ 功能上量到的 24% 可迁移)是共享的,系数插值预测新 Tx 的增量不比平均好——参数空间里没有共享的低秩基底。
  结论:一份适配只能服务照亮同一批表面的 Tx;多 Tx 部署应存视觉几何 + 每 Tx 1–3% 高斯的几何增量,不共用一份适配几何,也不从独立适配里取基底。
- **解码精度是主指标,且要按分布报**:方位角以 (cos, sin) 双通道编码后,解码误差中位数 **0.6°**(P90 5.9°,RMSE 9.8°;单通道版 1.8° / 11° / 19°)、天顶 0.9°(P90 12°)、
  时延 2.3 ns(P90 11.5 ns);RMSE 由 3% 的双径像素拖着,目标本身在这些像素上就带 180° 级误差。教程的角度×幅度编码解码中位数 27°,
  三成像素 > 45°——除法解码器在低幅度区无界。σ 扫描(0.33 / 1 / 2 / 4.3°)的极小点**随度量移动**:按像素计、各自支撑集 2°,固定支撑集 1°;
  按功率加权(每个像素乘自己的线性功率,不划支撑集)则方位随 σ 单调变好,在网格分辨率 0.33° 处最好(中位 0.84°,P90 2.15°),天顶相反。
  "σ* = 表示带宽"这条假设撤回;能说的是带度量的两句:按功率,场把强路径方位复现到 0.33° 核之下(细于 64×64 UPA 的 1.59° 波束宽);
  按像素计,内点极小在 1–2°。论文的 5.94° 是 M=10 波束指向误差,不是同一量。PSNR 只是 jet 折算的兼容指标。
...
### 7.1 子 6 GHz 的材料电导率单位错误(独立一节,论文里单独写)

- **错在哪**:教程 cell 6 对 ITU 类材料(石膏板、混凝土、玻璃、木头、天花板、刨花板、大理石)用
  `σ = a · (scene_freq / 1e-9)^b` 算电导率,分母应是 1e9(频率换成 GHz)。频率被乘了 1e18,σ 落到 1e16–1e24 S/m,
  这些材料在任何频率下都成了完美导体;塑料/皮革/布(σ 手写为 0)和金属不受影响。修成 f(GHz) 后 σ 回到 ITU-R P.2040 的 0.09–3.7 S/m。
- **它改变什么**(同一 Tx/Rx、depth 1、1e6 采样):路径数 6.7k(as-is)vs 104k(修正)vs 311k(统一 0.7);总功率几乎不变(LoS 主导)。
  多跳份额(depth 3 对 depth 1,8 个位置,2.4 GHz):as-is 下 depth-1 之外的功率均值 31%、三个位置 45–99%、LoS 只占 0.1–3%;
  修正后均值 13.7%、中位 7.7%、两个远端位置 38–41%。**as-is 变体下 depth 1 的设定本身不成立**;修正后 depth 1 对大多数位置够用,
  远端位置漏四成。60 GHz 统一散射(Tx 实验的设定)depth 1 拿到 ≥ 87% 功率,不受此影响。
- **与发布数据的关系**:发布的 `cbf_power.csv` 与我们 2.4 GHz 的计算值差 1.6 dB(60 GHz 差 34 dB),所以发布数据是 2.4 GHz 的;
  它对应哪一种材料变体,靠功率量级分不出来(两种变体总功率相差 < 1 dB)。论文正文写的 60 GHz 与数据不符,这一条与材料错误是两件事。
- **我们的处理**:`sionna_port/tutorial_materials.py` 提供 `asis` 与 `fixed` 两种变体,生成器 `--materials {uniform,tutorial,tutorial-asis}`;
  本工作的 2.4 GHz 数据集全部用修正版;今后 2.4 GHz 数据集建议 `--max-depth 3`。
...
## 8. 阶段 2 后半(2026-09-19 → 09-20)结果索引;冻结点 `d359dc9`

每一条给结论、关键数、以及 [stage2_notes.md](stage2_notes.md) 里的节号;论文文本在 [paper1_draft.md](paper1_draft.md)(v0.6)。

| 主题 | 结论 | 关键数 | notes |
| --- | --- | --- | --- |
| 跨视角一致性(主线) | 逐图归一化、逐视角 RT seed 两种违反都比任何表示改动代价大;波束谱对 seed 不敏感,把主张限定到目标类型 | −6.2 dB PSNR;路径级功率 RMSE 12.67 → 8.57(−32%);MVDR 18.75/3.91 → 18.69/3.94 | 第十轮 §1、§1b;第九轮 §4 |
| 几何适配的结构 | 增益分布式、低维、Tx 专属;均值只动 6 mm(是投影核 / 可见性补偿,不是 RF 表面恢复) | 随机 1% 高斯拿 73%;十组形变 5 个主成分 66%;可迁移 24% / 专属 76% | 第八轮 A 与对照;第九轮 §1 |
| 转移曲线(大堂) | 二元分区为主,指数拟合撤回为模型;平台 −0.46 dB 是硬数 | 同侧 4 源 +0.33 均,其余 6 源 −0.20 均;d_c 自助区间 2–40 m | 第八轮 T1 完成;第九轮 §6 |
| **走廊解离实验** | 距离与点亮表面重叠在大堂共线,走廊把它们拆开:共墙隔壁 3.7 m −11.4 dB,门对门 7.3 m +2.5 dB;符号由 IoU 是否为零决定 | 分离跨度 15.7 dB(大堂 0.9);seed 底 0.31 dB/次;同房 +2.39 与正对房 +2.47 相等 | 第十四轮 §10、§5(round 15) |
| 指标失明四实例 | 材料单位错、载频 34 dB、depth-1 缺失功率、σ 的最优值随度量移动——PSNR 都不动 | 多跳份额 13.7% → 31%;功率加权 σ* 在 0.33°、像素计 1° | 第十轮 §3;E3/E4;第九轮 §3 |
| 解码地板 / 天花板 | 拷贝最近邻在天顶、时延中位上赢场;场的收益在尾巴与稀疏采样 | 拷贝 1.07° / 0.79° / 0.86 ns vs 场 0.59° / 0.94° / 2.42 ns;上限 0.20° | 第十一轮 §1 |
| 密度交叉点(大堂) | 超过 ≈ 0.3 m 三通道场胜;两种子采样规则相同;可报两位 | 天顶 0.26 ± 0.005 m,时延 0.25 ± 0.002 m(4 seed);解码中位 sd 0.002–0.026° | 第十三轮 §1′;第十四轮 §3′;round 15 §7 |
| 密度交叉点(走廊,按 regime) | 走廊波导:方位 ≈ 0.5–0.6 m、时延不交叉;厅与房间:< 0.44–0.48 m。聚合会把相反 regime 平均掉;预测 #2 走廊反向、房间未分辨 | 全部 452 张(300 张子集偏向房间,已作废重算) | round 15 §2 |
| 整间房留出(预测 #3) | 拷贝崩溃(173° / 89° / 46 ns),场 7.1° / 8.0° / 6.5 ns;天顶输给常数预测 3.9° | 最近训练点 2.19 m;每图 ≈ 2k 个有路径像素 | round 15 §4 |
| 时延分解 | 解析范围 + 学习残差:直接学 8.93 → D 6.93 → ED 5.26 → ED + sec θ 4.97 ns RMSE;中位 2.42 → 0.55 | euclid 判据:中位 / P90 过,RMSE < 4.5 未过;单路径像素 RMSE 3.73 → 3.46 | 第十二轮 §4;第十四轮 §2c;round 15 §3b–3d |
| 几何对 mesh | 大堂中位深度误差 0.117 m(0.39 ns),走廊 0.365 m(1.22 ns,3.1×);高斯在表面前方 −0.22 / −0.48 m,= 时延负偏差的来源 | 范围项缺口 (range − z)/c 大堂均值 2.18 ns、P90 4.59 | round 15 §3 |
| Oracle 深度 | 测试时换真值深度三个 run 都变差,偏移 ≈ 几何平均偏差:残差在训练时已补偿几何,替换算两遍;不再做训练时 oracle | +0.92(大堂)/ +1.76 ns(走廊) | round 15 §3e、§8 |
| 场景 2 一致性 | 两个量级同号:归一化 −5.2 dB(大堂 −6.2),seed +17%(大堂 +48%) | | 第十四轮 §9 |
| Stage 1 视觉质量 | 大堂 39.4 dB(训练帧);走廊 25.3 dB 对 64 spp 目标(噪声底 27.2)、28.8 dB 对 512 spp 干净重渲;前两遍失败是相机帧转了 180° | 582,363 vs 1,014,142 高斯 | 第十四轮 §6、§6b |
| 数据 / 版本 / 纪律 | 生成器 G1 → G3 版本审计;渲染集不入库(manifest);全留出图默认落盘、渲染不全拒绝打分;分层抽样 | 2.1 GB PNG 从未推送历史剥离,备份分支保留 | data_version_audit.md;round 15 §1、§6 |
