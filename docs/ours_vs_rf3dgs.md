...
- **几何解冻**:db 模型 18.75 / 3.91 dB → 21.53 / 2.85 dB,本阶段最大单项收益。位移诊断:位置中位数只动 6 mm、最大 21 cm,
  ...
  **对 Tx 泛化论据的含义**:高斯位置是 Tx 不变的基底;可迁移的形状修正只值 0.24 dB(与颜色热启动同量级);
  76% 不可迁移是"颜色函数里没有入射方向"这个缺失维度的实测代价,也是 Tx 条件化传输的立项依据。
- **解码精度是主指标,且要按分布报**:方位角以 (cos, sin) 双通道编码后,解码误差中位数 **0.6°**(P90 5.9°,RMSE 9.8°;单通道版 1.8° / 11° / 19°)、天顶 0.9°(P90 12°)、
  ...
  三成像素 > 45°——除法解码器在低幅度区无界。σ 折算:1 px = 1/3°,数据集用的 σ=1°;M=10 阵列 θ3dB ≈ 10°,匹配应取 ≈ 4°。PSNR 只是 jet 折算的兼容指标。
