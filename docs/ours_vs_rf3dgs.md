...
- **几何解冻**:db 模型 18.75 / 3.91 dB → 21.53 / 2.85 dB,本阶段最大单项收益。位移诊断:位置中位数只动 6 mm、最大 21 cm,
  ...
  **搬多远**(10 个源 Tx 的转移曲线,2k 步预算,噪声底 0.03 dB):同侧 1.8 m +0.57、4.2 m +0.34、5.8 m +0.29 dB;
  大厅/南/西的六个源不论远近都是 −0.06…−0.35 dB;拟合 d_c = 5.3 m 且平台为负(−0.46 dB)。增益与距离 −0.81、与"同照亮表面"的 IoU +0.85,
  两者都需要。结论:一份适配只能服务照亮同一批表面的 Tx(同区域 ≈ 5 m),多 Tx 部署应存视觉几何 + 每 Tx 1–3% 高斯的几何增量。
- **解码精度是主指标,且要按分布报**:方位角以 (cos, sin) 双通道编码后,解码误差中位数 **0.6°**(P90 5.9°,RMSE 9.8°;单通道版 1.8° / 11° / 19°)、天顶 0.9°(P90 12°)、
  ...
  三成像素 > 45°——除法解码器在低幅度区无界。σ 扫描(0.33 / 1 / 2 / 4.3°)在**同一支撑集**(σ=0.33° 的 8.8M 像素)上重打分后,
  方位极小点在 **σ* = 1°**(中位 0.56°,P90 3.9°),不是各自支撑集给出的 2°(核越宽多出来的像素在中位数上更容易,把极小点推了过去);
  天顶从 1° 起持平(尾巴 2° 最好),时延从 1° 起持平。"σ* 是表示带宽、折算 θ3dB = 2.355σ* ≈ 2.4°(M≈43)或 4.7°(M≈22)"
  只是假设:数值随支撑集移动,论文的 5.94° 是 M=10 波束指向误差,不是同一量。PSNR 只是 jet 折算的兼容指标。
